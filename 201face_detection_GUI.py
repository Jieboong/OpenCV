...
face_cascade_name = 'C:/Users/user/anaconda3/envs/Vision/Library/etc/haarcascades/haarcascade_frontalface_alt.xml'
eyes_cascade_name = 'C:/Users/user/anaconda3/envs/Vision/Library/etc/haarcascades/haarcascade_eye_tree_eyeglasses.xml'
...
read_image = cv2.imread("./image/marathon_03.jpg")
(height, width) = read_image.shape[:2]
ratio = frame_width / width
dimension = (frame_width, int(height * ratio))
read_image = cv2.resize(read_image, dimension, interpolation=cv2.INTER_AREA)

image = cv2.cvtColor(read_image, cv2.COLOR_BGR2RGB) #RGB로 보여주기 convert
image = Image.fromarray(image)
imgtk = ImageTk.PhotoImage(image=image)
